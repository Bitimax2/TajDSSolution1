		...
		var pignoseLayerSlider = {
			init : function(options) {
				...
				return this.each(function() {
					...
					if(typeof _this.settings.prev !== 'undefined' &&
					   _this.settings.play !== null
						...
						$target.find(_this.settings.prev).unbind('click.slideControlHandler').bind('click.slideControlHandler', function(event) {
							pignoseLayerSlider.move.apply(_this, new Array(((_this.settings.direction == 'right')? 'left':'right'), 1, true));
							event.preventDefault();
						});
					}
					...
					$visual.append('<div class="slide_tint right">&nbsp;</div>')
						   .append('<div class="slide_tint left">&nbsp;</div>');
					...
					$slide.add($script).each(function() {
						...
						$(this).css({
							...
							marginRight : -_this[$(this).data('type')].offset - ($li.outerWidth(true)) + 'px',
							overflow   : 'hidden'
						})
						.children('li').css({
							display    : 'block',
				   ...
				   this.settings.controlAnim == false
					...
					if(direction == 'left') {
						...
						this.$slide.add(this.$script).each(function() {
							var $this = $(this);
							setTimeout(function() {
								$this.stop(true, true).animate({marginRight : -(_this[$this.data('type')].offset + _this[$this.data('type')].widthOnce * (1 + index)) + 'px'}, {
									duration : _this.settings.speed,
									easing   : _this.settings.easing,
									complete : function() {
										$(this).children('li').slice(0, index).appendTo($this);
										$(this).css({marginRight : -_this[$this.data('type')].offset - _this[$this.data('type')].widthOnce + 'px'});
